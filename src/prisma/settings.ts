...
export const getUserSettings = cache(
  async (userId: string): Promise<UserSettings> => {
    const row = await db.orm.public.UserSettings.where({ userId })
      .select("monthStartDay")
      .first();
    let trackingStartPeriod: string | null = null;
    try {
      const trackingRow = await db.orm.public.UserSettings.where({ userId })
        .select("trackingStartPeriod")
        .first();
...
export async function setMonthStartDay(
  userId: string,
  monthStartDay: number,
): Promise<void> {
  const clamped = Math.min(Math.max(Math.round(monthStartDay), 1), 28);
  const existing = await db.orm.public.UserSettings.where({ userId })
    .select("id")
    .first();
  if (existing) {
    await db.orm.public.UserSettings.where({ userId }).update({
      monthStartDay: clamped,
    });
  } else {
...
export async function setTrackingStartPeriod(
  userId: string,
  period: string | null,
): Promise<void> {
  ...
  if (existing) {
    ...
    await db.orm.public.UserSettings.create({
      userId,
      trackingStartPeriod: period,
    });
  }
