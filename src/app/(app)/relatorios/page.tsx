...
export default async function RelatoriosPage({
  ...
  const floorPeriod =
    trackingStartPeriod && trackingStartPeriod > accountPeriod
      ? trackingStartPeriod
      : accountPeriod;
  ...
  const barPeriods = Array.from({ length: range }, (_, i) =>
    addMonths(period, -(range - 1 - i)),
  ).filter((p) => p >= floorPeriod);
  const barData = await Promise.all(
    barPeriods.map((p) =>
      loadMonthData(user.id, p, monthStartDay, trackingStartPeriod),
    ),
  );
  ...
  const effectiveSummaries = barData.map((m) =>
    computeMonthSummary(m.transactions, considerBudget ? m.budgetProgress : []),
  );
    ...
    previousIndexForSummary >= 0
      ...
      : previous
        ? computeMonthSummary(
            ...
            considerBudget ? previous.budgetProgress : [],
          )
        : undefined;

  const currentExpenses = current.transactions.filter(
    (t) => t.type === "expense",
  );
  const previousExpenses = previous
    ? previous.transactions.filter((t) => t.type === "expense")
    : [];
  ...
  const filteredCategory = categoryId
    ? current.categories.find((c) => c.id === categoryId)
    : undefined;
  ...
  const changePct =
    previousSummary && previousSummary.previstoTotal > 0
      ? Math.round(
          ((currentSummary.previstoTotal - previousSummary.previstoTotal) /
            previousSummary.previstoTotal) *
            100,
        )
      : null;
  ...
  const average =
    monthsWithData.length > 0
      ? monthsWithData.reduce((sum, s) => sum + s.previstoTotal, 0) /
        monthsWithData.length
      : 0;

  const evolutionValues = barPeriods.map((p, i) => {
    if (!categoryId) return effectiveSummaries[i].previstoTotal;
    const periodExpenses = barData[i].transactions.filter(
      (t) => t.type === "expense",
    );
  ...
  const itemLabel =
    breakdown.mode === "category" ? "Categoria" : "Subcategoria";
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="app-metrics-grid grid grid-cols-3 gap-4">
        <div className={cn(cardStyle, "border border-border bg-surface")}>
          <div className={labelStyle}>
            {filteredCategory
              ? `${itemLabel} com maior gasto em ${filteredCategory.name}`
              : `${itemLabel} com maior gasto`}
          </div>
          <div className="mt-1 text-[17px] font-bold">
            {topRow?.name ?? "—"}
          </div>
          <div className="mt-0.5 text-xs text-text-secondary">
            {breakdown.total > 0 && topRow
              ? Math.round((topRow.total / breakdown.total) * 100)
              : 0}
            % do total
          </div>
        </div>
        ...
        <div className={cn(cardStyle, "border-none bg-primary")}>
          ...
          <div className="mt-0.5 text-xs text-[rgba(255,255,255,0.8)]">
            {barPeriods.length > 1
              ? `últimos ${barPeriods.length} meses`
              : "este mês"}
          </div>
        </div>
      </div>
      ...
      <div className="report-summary-grid">
        <div className="rounded-lg border border-border bg-surface p-5">
          <div className="mb-4 text-sm font-semibold">
            {filteredCategory
              ? `Gastos por subcategoria em ${filteredCategory.name}`
            ...
            <div className="flex flex-wrap items-center gap-5">
              <div className="relative h-[120px] w-[120px] shrink-0">
                <div
                  ...
                  style={{ background: breakdown.pieGradient }}
                />
                ...
                {breakdown.rows.map((r) => (
                  ...
                  >
                    ...
                    <span className="text-text-secondary">
                      {breakdown.total > 0
                        ? Math.round((r.total / breakdown.total) * 100)
                        : 0}
                      %
                    </span>
                  </div>
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-surface">
        ...
        {breakdown.rows.length === 0 ? (
          <div className="px-5 py-6 text-center text-[13px] text-text-secondary">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[480px] border-collapse">
              <thead>
                <tr>
                  <th className={cn(theadCellStyle, "text-left")}>
                    {itemLabel.toUpperCase()}
                  </th>
                  <th className={cn(theadCellStyle, "text-right")}>
                    TRANSAÇÕES
                  </th>
                  <th className={cn(theadCellStyle, "text-right")}>
                    VALOR TOTAL
              ...
              <tbody>
                {breakdown.rows.map((r) => (
                  <tr key={r.id} className="border-t border-border-soft">
                    ...
                    <td className={cn(tdCellStyle, "text-right font-bold")}>
                      {formatBRL(r.total)}
                    </td>
...
const theadCellStyle =
  "px-5 py-2.5 text-[11px] font-bold tracking-[0.4px] text-text-disabled";
